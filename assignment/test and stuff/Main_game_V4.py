...
def start():
    global playing, title, start_time, bomb_spawn_seconds, laser_spawn_time
    playing = True
    title = False
...
while running:
    ...
    if title:
        ...
        surface.blit(font1.render(f"Bomb mayhem best time = {bomb_mayhem_best_time/1000} seconds!", True, (0, 0, 0)), (screen_width - 750, screen_height - 20))
    ...
    if playing:
        ...
        for bomb in bombs:
            ...
            if bomb[2] >= 6:
                if bomb_mayhem != True:
                    surface_rotation = random.uniform(-1, 1)
                ...
                surface_rotation = 0
        try:
            if bomb_mayhem == True and bombs[0][2] >= 6:
                surface_rotation = random.uniform(-1, 1)
        except:
            pass
